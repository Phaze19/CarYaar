    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#000000' }} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 100 }} className="bg-black">
        ...
        <Animated.View entering={FadeInDown.duration(600).springify()} className="mb-8 mt-2">
          <Text className="text-neutral-400 font-medium text-sm mb-1" style={{ fontFamily: 'Poppins_500Medium' }}>Past Routes</Text>
        ...
        <Animated.View entering={FadeInUp.duration(800).delay(100).springify()} className="gap-4">
          {pastTrips.length === 0 ? (
            <View className="p-8 border border-neutral-800 border-dashed rounded-3xl items-center justify-center bg-neutral-900/80 mt-4">
              <Text className="text-neutral-400 font-medium" style={{ fontFamily: 'Poppins_500Medium' }}>No past trips found.</Text>
            </View>
          ) : (
            pastTrips.map((trip, index) => (
              <Animated.View 
                ...
                className="flex-row justify-between items-center bg-neutral-900/80 border border-neutral-800 p-5 rounded-3xl"
              >
                <View>
                  <Text className="text-xs text-neutral-400 font-medium mb-1" style={{ fontFamily: 'Poppins_500Medium' }}>{new Date(trip.date).toLocaleDateString()}</Text>
                  <Text className="text-lg font-bold text-white tracking-tight" style={{ fontFamily: 'Poppins_600SemiBold' }}>{trip.distance_km} km</Text>
                  <View className={`self-start px-2 py-0.5 rounded-md mt-1 ${trip.isDriver ? 'bg-yellow-400/20' : 'bg-neutral-800'}`}>
                    <Text className={`font-semibold text-[10px] uppercase ${trip.isDriver ? 'text-yellow-400' : 'text-neutral-400'}`} style={{ fontFamily: 'Poppins_600SemiBold' }}>

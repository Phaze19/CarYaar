    ...
    <SafeAreaView style={{ flex: 1 }} edges={['top']} className="bg-slate-900">
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        ...
        <Animated.View entering={FadeInDown.duration(600).springify()} className="mb-8 mt-2">
          <Text className="text-slate-400 font-medium text-sm mb-1">Past Routes</Text>
        ...
        <Animated.View entering={FadeInUp.duration(800).delay(100).springify()} className="gap-4">
          {pastTrips.length === 0 ? (
            <View className="p-8 border border-slate-700/50 border-dashed rounded-3xl items-center justify-center bg-slate-800/80 mt-4">
              <Text className="text-slate-400 font-medium">No past trips found.</Text>
            </View>
          ) : (
            pastTrips.map((trip, index) => (
              ...
              >
                <View>
                  <Text className="text-xs text-slate-400 font-medium mb-1">{new Date(trip.date).toLocaleDateString()}</Text>
                  <Text className="text-lg font-bold text-white tracking-tight">{trip.distance_km} km</Text>
                  <View className={`self-start px-2 py-0.5 rounded-md mt-1 ${trip.isDriver ? 'bg-cyan-500/20' : 'bg-slate-700/50'}`}>
                    <Text className={`font-semibold text-[10px] uppercase ${trip.isDriver ? 'text-cyan-400' : 'text-slate-300'}`}>
                      {trip.isDriver ? 'Driver' : 'Passenger'}
                    </Text>
                  </View>
                </View>
                <View className="items-end">
                  <Text className="font-bold text-xl text-white tracking-tight">₹{trip.personalShare.toFixed(2)}</Text>
                </View>
              </Animated.View>
            ))
          )}
